...
export default function AppHeader() {
  ...
  return (
    <header className="border-b border-slate-200/60 bg-white/70 backdrop-blur-md sticky top-0 z-50">
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-4 sm:px-6 py-4">
        ...
        >
          ...
          <div className="flex flex-col">
            <span className="text-lg font-bold tracking-tight text-slate-900 leading-tight">{t("nav.intranet")}</span>
            <span className="text-[10px] font-medium uppercase tracking-widest text-slate-500 leading-tight">ECLA DevClub</span>
          </div>
        </Link>
          ...
          <div className="ml-2 flex items-center gap-3 pl-2">
            {isLoaded && user && (
              <div className="flex items-center gap-3 text-right">
                ...
                <div className="flex h-9 w-9 items-center justify-center rounded-full bg-slate-100 text-sm font-bold text-slate-600 ring-2 ring-slate-200">
            ...
            <button
              ...
              className="ml-2 rounded-full p-2 text-slate-400 hover:bg-slate-100 hover:text-slate-700 transition"
              title={t("nav.logout")}
            >
      ...
      {(isMenuOpen || isClosing) && (
        <div className={`border-t border-slate-100 bg-white/95 px-6 py-4 md:hidden shadow-xl backdrop-blur-md ${isClosing ? "animate-slide-up" : "animate-slide-down"}`}>
          {isLoaded && user && (
          ...
          <nav className="flex flex-col gap-2 text-sm font-medium text-slate-600">
            <Link href="/" className="rounded-lg p-2 hover:bg-slate-50" onClick={closeMenu}>{t("nav.intranet")}</Link>
            <Link href="/departments" className="rounded-lg p-2 hover:bg-slate-50" onClick={closeMenu}>{t("nav.departments")}</Link>
            <Link href="/employees" className="rounded-lg p-2 hover:bg-slate-50" onClick={closeMenu}>{t("nav.employees")}</Link>
            <Link href="/tickets" className="rounded-lg p-2 hover:bg-slate-50" onClick={closeMenu}>{t("nav.tickets")}</Link>
            <Link href="/documents" className="rounded-lg p-2 hover:bg-slate-50" onClick={closeMenu}>{t("nav.documents")}</Link>
            
            <div className="my-2 border-t border-slate-100" />
            
            <div className="flex items-center justify-between p-2">
              <span className="text-xs font-medium text-slate-400 uppercase tracking-widest">{t("nav.settings")}</span>
